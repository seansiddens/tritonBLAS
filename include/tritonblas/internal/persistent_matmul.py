...
from .tessera.tessera import transform, chiplet_transform, transform_depth3
import torch
...
@triton.jit()
def persistent_matmul_tessera_depth3(
    A,
    B,
    C,
    bias_ptr,
    ...
    BLOCK_SIZE_N: tl.constexpr,
    BLOCK_SIZE_K: tl.constexpr,
    ordering0: tl.constexpr, # Grid level ordering
    ordering1: tl.constexpr, # L3 level ordering
    ordering2: tl.constexpr, # L2 level ordering
    L3Y,
    L3X,
    L2Y,
    L2X,
    ...
    for tile_id in range(pid, total_tiles, NUM_SMS):
        transformed_pid = transform_depth3(tile_id, num_pid_m, num_pid_n, ordering0, ordering1, ordering2, L3Y, L3X, L2Y, L2X)
        pid_m = transformed_pid // num_pid_n
        pid_n = transformed_pid % num_pid_n
        tl.assume(pid_m >= 0)
        tl.assume(pid_n >= 0)
